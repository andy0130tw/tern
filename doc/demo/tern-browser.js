(function (global, factory) {
  ...
  var acorn_loose = /*#__PURE__*/Object.freeze({
    __proto__: null,
  ...
  var acorn_walk = /*#__PURE__*/Object.freeze({
    __proto__: null,
    ancestor: ancestor,
    base: base,
  ...
  //   tern.def = {init: mod};
  var init = function init(exports, infer) {
    ...
    TypeParser.prototype = {
      eat: function eat(str) {
        if (str.length == 1 ? this.spec.charAt(this.pos) == str : this.spec.indexOf(str, this.pos) == this.pos) {
          this.pos += str.length;
          return true;
        }
      },
      word: function word(re) {
        var word = "",
            ch,
            re = re || /[\w$]/;
      ...
      parseFnType: function parseFnType(comp, name, top, generator) {
        var args = [],
            names = [],
            computed = false;
        ...
        if (top && (fn = this.base)) infer.Fn.call(this.base, name, infer.ANull, args, names, retType, generator);else fn = new infer.Fn(name, infer.ANull, args, names, retType, generator);
        if (computeRet) fn.computeRet = computeRet;
        if (computeRetStart != null) fn.computeRetSource = this.spec.slice(computeRetStart, this.pos);
        return fn;
      },
      parseType: function parseType(comp, name, top) {
      ...
      parseTypeMaybeProp: function parseTypeMaybeProp(comp, name, top) {
      ...
      extendWithProp: function extendWithProp(base) {
      ...
      parseTypeInner: function parseTypeInner(comp, name, top) {
        var gen;
          ...
          if (!this.eat("}")) {
            for (var i = 0;; ++i) {
              ...
              if (colon != -1) {
                ...
                if (/^[$\w?]+$/.test(propName)) this.pos = colon + 2;else propName = null;
              }
      ...
      fromWord: function fromWord(spec) {
        var cx = infer.cx();
      ...
      parsePoly: function parsePoly(base) {
        var propName = "<i>",
            match;
    ...
    function addArgCallEffect(type, argNum) {
      addEffect(type, function (_self, args) {
        if (args[argNum]) args[argNum].propagate(new infer.IsCallee(infer.cx().topScope, type.args[argNum].args, null, infer.ANull));
      });
    }

    function parseType(spec, name, base, forceNew) {
      var type = new TypeParser(spec, null, base, forceNew).parseType(false, name, true);
      if (type instanceof infer.AVal) type.types.forEach(addArgCallEffects);else addArgCallEffects(type);
      return type;
    }
      ...
      if (effect.indexOf("propagate ") == 0) {
        ...
        addEffect(fn, function (self, args) {
          ...
          from.forAllProps(function (prop, val, local) {
            if (local && prop != "<i>") to.propagate(new infer.DefProp(prop, val));
          });
        });
      } else {
    ...
    function descendProps(base, parts) {
      for (var i = 0; i < parts.length && base != infer.ANull; ++i) {
        ...
        } else if (base instanceof infer.Obj && (prop == "prototype" && base instanceof infer.Fn || base.hasProp(prop))) {
    ...
    function passOne(base, spec, path) {
      if (!base) {
        ...
        if (tp) {
          if (/^fn\(/.test(tp)) base = emptyObj(infer.Fn);else if (tp.charAt(0) == "[") base = emptyObj(infer.Arr);else if (tp.charAt(0) == "+") base = emptyObj(infer.Obj);else throw new Error("Invalid !type spec: " + tp);
    ...
    var IsCreated = infer.constraint({
      construct: function construct(created, target, spec) {
        this.created = created;
        this.target = target;
        this.spec = spec;
      },
      addType: function addType(tp) {
    ...
    var IsBound = infer.constraint({
      construct: function construct(self, args, target) {
        this.self = self;
        this.args = args;
        this.target = target;
      },
      addType: function addType(tp) {
        ...
        this.target.addType(new infer.Fn(tp.name, infer.ANull, tp.args.slice(this.args.length), tp.argNames.slice(this.args.length), tp.retval, tp.generator));
    ...
    var PromiseResolvesTo = infer.constraint({
      construct: function construct(output) {
        this.output = output;
      },
      addType: function addType(tp) {
        if (tp.constructor == infer.Obj && tp.name == "Promise" && tp.hasProp(":t")) tp.getProp(":t").propagate(this.output);else tp.propagate(this.output);
      }
    });
    ...
    infer.registerFunction("Promise_then", function (self, args, argNodes) {
      ...
      var value = result.defProp(":t", argNodes && argNodes[0]),
          ty;
      if (fn.retval.isEmpty() && (ty = self.getType()) instanceof infer.Obj && ty.hasProp(":t")) ty.getProp(":t").propagate(value, WG_PROMISE_KEEP_VALUE);
    ...
    infer.registerFunction("getSymbol", function (_self, _args, argNodes) {
      if (argNodes && argNodes.length && argNodes[0].type == "Literal" && typeof argNodes[0].value == "string") return infer.getSymbol(argNodes[0].value);else return infer.ANull;
    });
    return exports;
  };
  ...
  var def$1 = /*#__PURE__*/Object.freeze(/*#__PURE__*/Object.assign(/*#__PURE__*/Object.create(null), def, {
    'default': def,
    init: init
  }));
  ...
  var signal$1 = /*#__PURE__*/Object.freeze(/*#__PURE__*/Object.assign(/*#__PURE__*/Object.create(null), signal_1, {
    'default': signal_1,
    mixin: mixin
  }));

  var acorn = /*@__PURE__*/getAugmentedNamespace(_acorn);

  var acorn_loose$1 = /*@__PURE__*/getAugmentedNamespace(acorn_loose);

  var walk = /*@__PURE__*/getAugmentedNamespace(acorn_walk);
  ...
  var infer_1 = createCommonjsModule(function (module, exports) {
    ...
    // single type.
    var _toString = exports.toString = function (type, maxDepth, parent) {
    ...
    var ANull = exports.ANull = signal_1.mixin({
      ...
      getFunctionType: function getFunctionType() {},
      getObjType: function getObjType() {},
      getSymbolType: function getSymbolType() {},
      getType: function getType() {},
      gatherProperties: function gatherProperties() {},
      propagatesTo: function propagatesTo() {},
      typeHint: function typeHint() {},
      propHint: function propHint() {},
    ...
    AVal.prototype = extend(ANull, {
      addType: function addType(type, weight) {
      ...
      propagate: function propagate(target, weight) {
      ...
      forAllProps: function forAllProps(c) {
      ...
      getType: function getType(guess) {
        if (this.types.length === 0 && guess !== false) return this.makeupType();
        if (this.types.length === 1) return this.types[0];
        return canonicalType(this.types);
      },
      toString: function toString(maxDepth, parent) {
        if (this.types.length == 0) return _toString(this.makeupType(), maxDepth, parent);
        if (this.types.length == 1) return _toString(this.types[0], maxDepth, parent);
        var simplified = simplifyTypes(this.types);
        if (simplified.length > 2) return "?";
        return simplified.map(function (tp) {
          return _toString(tp, maxDepth, parent);
        }).join("|");
      },
      makeupPropType: function makeupPropType(obj) {
      ...
      makeupType: function makeupType() {
      ...
      gatherProperties: function gatherProperties(f, depth) {
        for (var i = 0; i < this.types.length; ++i) {
          this.types[i].gatherProperties(f, depth);
        }
      },
      guessProperties: function guessProperties(f) {
    ...
    var GetProp = constraint({
      construct: function construct(prop, target) {
        this.prop = prop;
        this.target = target;
      },
      addType: function addType(type, weight) {
    ...
    var DefProp = exports.PropHasSubset = exports.DefProp = constraint({
      construct: function construct(prop, type, originNode) {
        this.prop = prop;
        this.type = type;
    ...
    var IsCallee = exports.IsCallee = constraint({
      construct: function construct(self, args, argNodes, retval) {
        this.self = self;
        this.args = args;
      ...
      addType: function addType(fn, weight) {
    ...
    var HasMethodCall = constraint({
      construct: function construct(propName, args, argNodes, retval) {
      ...
      addType: function addType(obj, weight) {
    ...
    var IsCtor = exports.IsCtor = constraint({
      construct: function construct(target, noReuse) {
        this.target = target;
        this.noReuse = noReuse;
      },
      addType: function addType(f, weight) {
    ...
    var IsProto = exports.IsProto = constraint({
      construct: function construct(ctor, target) {
        this.ctor = ctor;
        this.target = target;
      },
      addType: function addType(o, _weight) {
    ...
    var FnPrototype = constraint({
      construct: function construct(fn) {
        this.fn = fn;
      },
      addType: function addType(o, _weight) {
    ...
    var IsAdded = constraint({
      construct: function construct(other, target) {
        this.other = other;
        this.target = target;
      },
      addType: function addType(type, weight) {
    ...
    var IfObj = exports.IfObj = constraint({
      ...
      addType: function addType(t, weight) {
    ...
    var SpeculativeThis = constraint({
      construct: function construct(obj, ctor) {
        this.obj = obj;
        this.ctor = ctor;
      },
      addType: function addType(tp) {
    ...
    var Muffle = constraint({
      construct: function construct(inner, weight) {
        this.inner = inner;
        this.weight = weight;
      },
      addType: function addType(tp, weight) {
    ...
    Type.prototype = extend(ANull, {
      constructor: Type,
      propagate: function propagate(c, w) {
        c.addType(this, w);
      },
      hasType: function hasType(other) {
        return other == this;
      },
      isEmpty: function isEmpty() {
        return false;
    ...
    Obj.prototype = extend(Type.prototype, {
      constructor: Obj,
      toString: function toString(maxDepth) {
        ...
        for (var prop in this.props) {
          if (prop != "<i>") {
            ...
            if (maxDepth) props.push(prop + ": " + _toString(this.props[prop], maxDepth - 1, this));else props.push(prop);
          }
        }
      ...
      hasProp: function hasProp(prop, searchProto) {
        if (isInteger(prop)) prop = this.normalizeIntegerProp(prop);
        var found = this.props[prop];
        if (searchProto !== false) for (var p = this.proto; p && !found; p = p.proto) {
          found = p.props[prop];
        }
        return found;
      },
      defProp: function defProp(prop, originNode) {
      ...
      normalizeIntegerProp: function normalizeIntegerProp(_) {
        return "<i>";
      },
      broadcastProp: function broadcastProp(prop, val, local) {
        if (local) {
      ...
      onProtoProp: function onProtoProp(prop, val, _local) {
      ...
      replaceProto: function replaceProto(proto) {
      ...
      ensureMaybeProps: function ensureMaybeProps() {
        if (!this.maybeProps) {
      ...
      maybeUnregProtoPropHandler: function maybeUnregProtoPropHandler() {
        if (this.maybeProps) {
      ...
      unregPropHandler: function unregPropHandler(handler) {
    ...
    Fn.prototype = extend(Obj.prototype, {
      ...
      toString: function toString(maxDepth) {
        ...
        for (var i = 0; i < this.args.length; ++i) {
          ...
          str += maxDepth > -3 ? _toString(this.args[i], maxDepth - 1, this) : "?";
        }

        str += ")";
        if (!this.retval.isEmpty()) str += " -> " + (maxDepth > -3 ? _toString(this.retval, maxDepth - 1, this) : "?");
        return str;
      },
      getProp: function getProp(prop) {
        if (prop == "prototype") {
      ...
      toString: function toString(maxDepth) {
        ...
        if (this.tuple) {
          ...
          for (var i = 0; (i in this.props); i++) {
            var type = _toString(this.getProp(String(i)), maxDepth - 1, this);
          ...
          content = _toString(this.getProp("<i>"), maxDepth - 1, this);
        }

        return "[" + content + "]";
      },
      normalizeIntegerProp: function normalizeIntegerProp(prop) {
    ...
    Sym.prototype = extend(Prim.prototype, {
      constructor: Sym,
      asPropName: function asPropName() {
        return ":" + this.symName;
      },
      getSymbolType: function getSymbolType() {
        return this;
    ...
    Scope.prototype = extend(Obj.prototype, {
      constructor: Scope,
      defVar: function defVar(name, originNode) {
    ...
    function maybeTagAsGeneric(fn) {
      ...
      if (foundPath) {
        ...
        var p = new def$1.TypeParser(foundPath);
    ...
    var scopeGatherer = exports.scopeGatherer = walk.make({
      VariablePattern: function VariablePattern(node, scopes) {
        if (scopes.inner) addVar(scopes.inner, node);
      },
      AssignmentPattern: function AssignmentPattern(node, scopes, c) {
        c(node.left, scopes, "Pattern");
        c(node.right, scopes.outer, "Expression");
      },
      AssignmentExpression: function AssignmentExpression(node, scope, c) {
      ...
      MemberPattern: function MemberPattern(node, scope, c) {
        c(node, scope.outer);
      },
      Function: function Function(node, scope, c) {
      ...
      BlockStatement: function BlockStatement(node, scope, c) {
      ...
      CatchClause: function CatchClause(node, scope, c) {
        if (!node.param) {
          return;
        }
      ...
      VariableDeclaration: function VariableDeclaration(node, scope, c) {
      ...
      ClassDeclaration: function ClassDeclaration(node, scope, c) {
      ...
      ForInStatement: function ForInStatement(node, scope, c) {
      ...
      ForStatement: function ForStatement(node, scope, c) {
    ...
    var inferPatternVisitor = exports.inferPatternVisitor = {
      Identifier: function Identifier(node, scope, source) {
        source.propagate(ensureVar(node, scope));
      },
      MemberExpression: function MemberExpression(node, scope, source) {
        var obj = infer(node.object, scope);
        var pName = propName(node, scope);
        obj.propagate(new DefProp(pName, source, node.property));
      },
      RestElement: function RestElement(node, scope, source) {
        connectPattern(node.argument, scope, new Arr(source));
      },
      ObjectPattern: function ObjectPattern(node, scope, source) {
      ...
      ArrayPattern: function ArrayPattern(node, scope, source) {
        for (var i = 0; i < node.elements.length; i++) {
          if (node.elements[i]) connectPattern(node.elements[i], scope, source.getProp(String(i)));
        }
      },
      AssignmentPattern: function AssignmentPattern(node, scope, source) {
    ...
    function arrayLiteralType(elements, scope, inner) {
      ...
      if (tuple) {
        ...
        for (var i = 0; i < elements.length; i++) {
          var elt = elements[i];
          if (!elt) tuple = false;else if (elt.type != "Literal" || litType && litType != _typeof_1(elt.value)) homogenous = false;else litType = _typeof_1(elt.value);
        }
    ...
    var inferWrapper = exports.inferWrapper = walk.make({
      Expression: function Expression(node, scope) {
        infer(node, node.scope || scope, ANull);
      },
      ObjectExpression: function ObjectExpression(node, scope) {
        infer(node, node.scope || scope, ANull);
      },
      FunctionDeclaration: function FunctionDeclaration(node, scope, c) {
        var inner = node.scope,
            fn = inner.fnType;
        connectParams(node, inner);
      ...
      Statement: function Statement(node, scope, c) {
        c(node, node.scope || scope);
      },
      ExportDefaultDeclaration: function ExportDefaultDeclaration(node, scope, c) {
      ...
      ForOfStatement: function ForOfStatement(node, scope, c) {
        var pattern = loopPattern(node.left),
            target;
    ...
    var parse = exports.parse = function (text, options, thirdArg) {
      ...
      try {
        ...
        ast = acorn_loose$1.parse(text, options);
      }
    ...
    var typeFinder = exports.typeFinder = {
      ...
      ThisExpression: function ThisExpression(_node, scope) {
        return getThis(scope);
      },
      ...
      YieldExpression: function YieldExpression(_node, scope) {
        var fn = functionScope(scope).fnType;
        return fn ? fn.yieldval : ANull;
      }
    };
    ...
    var searchVisitor = exports.searchVisitor = walk.make({
      Function: function Function(node, _st, c) {
        walk.base.Function(node, node.scope, c);
      },
      CatchClause: function CatchClause(node, _st, c) {
        walk.base.CatchClause(node, node.scope, c);
      },
      Property: function Property(node, st, c) {
        if (node.computed) c(node.key, st, "Expression");
        if (node.key != node.value) c(node.value, st, "Expression");
      },
      Statement: function Statement(node, st, c) {
        c(node, node.scope || st);
      },
      ImportSpecifier: function ImportSpecifier(node, st, c) {
        c(node.local, st);
      },
      ImportDefaultSpecifier: function ImportDefaultSpecifier(node, st, c) {
        c(node.local, st);
      },
      ImportNamespaceSpecifier: function ImportNamespaceSpecifier(node, st, c) {
        c(node.local, st);
      }
    });
    ...
    var findTypeFromContext = exports.findTypeFromContext = {
      ArrayExpression: function ArrayExpression(parent, _, get) {
        return get(parent, true).getProp("<i>");
      },
      ObjectExpression: function ObjectExpression(parent, node, get) {
        for (var i = 0; i < parent.properties.length; ++i) {
          var prop = node.properties[i];
          if (prop.value == node) return get(parent, true).getProp(propName(prop));
        }
      },
      UnaryExpression: function UnaryExpression(parent) {
        return unopResultType(parent.operator);
      },
      UpdateExpression: function UpdateExpression() {
        return cx.num;
      },
      BinaryExpression: function BinaryExpression(parent) {
        return binopIsBoolean(parent.operator) ? cx.bool : cx.num;
      },
      AssignmentExpression: function AssignmentExpression(parent, _, get) {
        return get(parent.left);
      },
      LogicalExpression: function LogicalExpression(parent, _, get) {
        return get(parent, true);
      },
      ConditionalExpression: function ConditionalExpression(parent, node, get) {
        if (parent.consequent == node || parent.alternate == node) return get(parent, true);
      },
      CallExpression: function CallExpression(parent, node, get) {
      ...
      ReturnStatement: function ReturnStatement(_parent, node, get) {
      ...
      VariableDeclarator: function VariableDeclarator(parent, node, get) {
    ...
    var simpleWalker = walk.make({
      Function: function Function(node, _scope, c) {
    ...
    var def$1 = exports.def = def.init({}, exports);
  });

  var infer = /*#__PURE__*/Object.freeze(/*#__PURE__*/Object.assign(/*#__PURE__*/Object.create(null), infer_1, {
    'default': infer_1
  }));

  var tern = createCommonjsModule(function (module, exports) {
    ...
    function parseFile(srv, file) {
      ...
      var ast = infer_1.parse(text, options);
    ...
    function updateText(file, text, srv) {
      ...
      infer_1.withContext(srv.cx, function () {
    ...
    var Server = exports.Server = function (options) {
      ...
      for (var plugin in options.plugins) {
        if (options.plugins.hasOwnProperty(plugin)) this.loadPlugin(plugin, options.plugins[plugin]);
      }

      this.reset();
    };

    Server.prototype = signal_1.mixin({
      ...
      reset: function reset() {
        this.signal("reset");
        this.cx = new infer_1.Context(this.defs, this);
        this.uses = 0;
        this.budgets = Object.create(null);
      ...
      request: function request(doc, c) {
      ...
      flush: function flush(c) {
        var cx = this.cx;
        analyzeAll(this, null, function (err) {
          if (err) return c(err);
          infer_1.withContext(cx, c);
        });
      },
      startAsyncAction: function startAsyncAction() {
        ++this.pending;
      },
      finishAsyncAction: function finishAsyncAction(err) {
        if (err) this.asyncError = err;
        if (--this.pending === 0) this.signal("everythingFetched");
      },
      addDefs: function addDefs(defs, toFront) {
        if (toFront) this.defs.unshift(defs);else this.defs.push(defs);
        if (this.cx) this.reset();
      },
      deleteDefs: function deleteDefs(name) {
      ...
      loadPlugin: function loadPlugin(name, options) {
        ...
        if (init.passes) for (var type in init.passes) {
          if (init.passes.hasOwnProperty(type)) this.on(type, init.passes[type]);
        }
      },
      normalizeFilename: function normalizeFilename(name) {
    ...
    function doRequest(srv, doc, c) {
      ...
      analyzeAll(srv, timeBudget, function (err) {
        ...
        infer_1.withContext(srv.cx, function () {
          ...
          try {
            if (timeBudget) infer_1.withTimeout(timeBudget[0], run);else run();
          } catch (e) {
    ...
    function analyzeFile(srv, file) {
      infer_1.withContext(srv.cx, function () {
        ...
        infer_1.analyze(file.ast, file.name, file.scope);
    ...
    function analyzeAll(srv, timeBudget, c) {
      ...
      if (srv.needsPurge.length > 0) infer_1.withContext(srv.cx, function () {
        infer_1.purge(srv.needsPurge);
      ...
      for (var i = 0; i < srv.files.length;) {
        ...
        for (var j = 0; j < toAnalyze.length; j++) {
          ...
          if (file.parent && !chargeOnBudget(srv, file)) {
            ...
            try {
              infer_1.withTimeout(timeBudget[0], function () {
                analyzeFile(srv, file);
              });
            } catch (e) {
              if (e instanceof infer_1.TimedOut) return c(e);else throw e;
            }
    ...
    function resolveFile(srv, localFiles, name) {
      ...
      infer_1.withContext(srv.cx, function () {
        infer_1.purge(file.name, pos, pos + file.text.length);
        var text = file.text,
            m;
        ...
        var scopeStart = infer_1.scopeAt(realFile.ast, pos, realFile.scope);
        var scopeEnd = infer_1.scopeAt(realFile.ast, pos + text.length, realFile.scope);
        var scope = file.scope = scopeDepth(scopeStart) < scopeDepth(scopeEnd) ? scopeEnd : scopeStart;
        file.ast = parseFile(srv, file);
        infer_1.analyze(file.ast, file.name, scope); // This is a kludge to tie together the function types (if any)
        // outside and inside of the fragment, so that arguments and
        // return values have some information known about them.

        tieTogether: {
          if (inObject || atFunction) {
            var newInner = infer_1.scopeAt(file.ast, line.length, scopeStart);
            ...
            if (inObject) {
              ...
              var inner = infer_1.scopeAt(realFile.ast, pos + line.length, realFile.scope);
    ...
    function isPosition(val) {
      return typeof val == "number" || _typeof_1(val) == "object" && typeof val.line == "number" && typeof val.ch == "number";
    ...
    function invalidDoc(doc) {
      ...
      if (doc.files) {
        ...
        for (var i = 0; i < doc.files.length; ++i) {
          ...
          if (_typeof_1(file) != "object") return ".files[n] must be objects";else if (typeof file.name != "string") return ".files[n].name must be a string";else if (file.type == "delete") continue;else if (typeof file.text != "string") return ".files[n].text must be a string";else if (file.type == "part") {
    ...
    function forwardCharacters(file, start, chars) {
      ...
      if (file.hasAstral) {
        ...
        while ((m = astral.exec(file.text)) && m.index < pos) {
          pos++;
        }
      }
    ...
    function charDistanceBetween(file, start, end) {
      ...
      if (file.hasAstral) {
        ...
        while ((m = astral.exec(file.text)) && m.index < end) {
          diff--;
        }
      }
    ...
    var addCompletion = exports.addCompletion = function (query, completions, name, aval, depth) {
      ...
      if (aval && typeInfo) {
        ...
        rec.guess = infer_1.didGuess();
        if (query.types) rec.type = infer_1.toString(aval);
    ...
    function findCompletions(srv, query, file) {
      ...
      if (query.expandWordForward !== false) while (wordEnd < text.length && acorn.isIdentifierChar(text.charCodeAt(wordEnd))) {
        ++wordEnd;
      }
      ...
      var exprAt = infer_1.findExpressionAround(file.ast, null, wordStart, file.scope);
      ...
      if (exprAt) {
        ...
        if (exprNode.type == "MemberExpression" && exprNode.object.end < wordStart) {
          ...
          var parent = infer_1.parentNode(exprNode, file.ast);
      ...
      if (objLit) {
        ...
        objType = infer_1.typeFromContext(file.ast, objLit);
        ignoreObj = objLit.node.objType;
      } else if (memberExpr) {
        prop = memberExpr.node.property;
        prop = prop.type == "Literal" ? prop.value.slice(1) : prop.name;
        memberExpr.node = memberExpr.node.object;
        objType = infer_1.expressionType(memberExpr);
      } else if (text.charAt(wordStart - 1) == ".") {
        var pathStart = wordStart - 1;

        while (pathStart && (text.charAt(pathStart - 1) == "." || acorn.isIdentifierChar(text.charCodeAt(pathStart - 1)))) {
          pathStart--;
        }
        ...
        if (path) {
          objType = infer_1.def.parsePath(path, file.scope).getObjType();
          prop = word;
        }
      }
      ...
      if (prop != null) {
        ...
        if (objType) infer_1.forAllPropertiesOf(objType, gather);
        ...
        if (!completions.length && word.length >= 2 && query.guess !== false) for (var prop in srv.cx.props) {
          gather(prop, srv.cx.props[prop][0], 0);
        }
        hookname = "memberCompletion";
      } else {
        infer_1.forAllLocalsAt(file.ast, wordStart, file.scope, gather);
    ...
    var findExpr = exports.findQueryExpr = function (file, query, wide) {
      ...
      if (query.variable) {
        var scope = infer_1.scopeAt(file.ast, resolvePos(file, query.end), file.scope);
        return {
          node: {
            type: "Identifier",
            name: query.variable,
        ...
        var expr = infer_1.findExpressionAt(file.ast, start, end, file.scope);

        if (!expr) {
          var span = infer_1.findClosestExpression(file.ast, start, end, file.scope);
          if (span && !inBody(span.node, end) && (wide || (start == null ? end : start) - span.node.start < 20 || span.node.end - end < 20)) expr = span;
        }

        if (!expr) {
          var around = infer_1.findExpressionAround(file.ast, start, end, file.scope);
    ...
    function ensureObj(tp) {
      if (!tp || !(tp = tp.getType()) || !(tp instanceof infer_1.Obj)) return null;
      return tp;
    }
    ...
    function findExprType(srv, query, file, expr) {
      ...
      if (expr) {
        infer_1.resetGuessing();
        type = infer_1.expressionType(expr);
      }
      ...
      if (expr.node.type == "ObjectExpression" && query.end != null && (objProp = pointInProp(expr.node, resolvePos(file, query.end)))) {
        ...
        var fromCx = ensureObj(infer_1.typeFromContext(file.ast, expr));
    ...
    function findTypeAt(srv, query, file) {
      ...
      var result = {
        guess: infer_1.didGuess(),
        type: infer_1.toString(exprType, query.depth),
        name: type && type.name,
        exprName: exprName,
        doc: exprType.doc,
        url: exprType.url
    ...
    function findRefsToVariable(srv, query, file, expr, isRename) {
      ...
      if (scope.originNode) {
        ...
        if (isRename) {
          ...
          if (prev) infer_1.findRefs(scope.originNode, scope, isRename, prev, function (node) {
            throw ternError("Renaming `" + name + "` to `" + isRename + "` would shadow the definition used at line " + (asLineChar(file, node.start).line + 1));
          });
        }

        infer_1.findRefs(scope.originNode, scope, name, scope, storeRef(file));
      } else {
        type = "global";

        if (query.onlySourceFile) {
          infer_1.findRefs(file.ast, file.scope, name, scope, storeRef(file));
        } else {
          for (var i = 0; i < srv.files.length; ++i) {
            var cur = srv.files[i];
            infer_1.findRefs(cur.ast, cur.scope, name, scope, storeRef(cur));
          }
        }
      }
    ...
    function findRefsToProperty(srv, query, sourceFile, expr, prop) {
      var exprType = infer_1.expressionType(expr);
      ...
      if (query.onlySourceFile) {
        infer_1.findPropRefs(sourceFile.ast, sourceFile.scope, objType, prop.name, storeRef(sourceFile));
      } else {
        for (var i = 0; i < srv.files.length; ++i) {
          var cur = srv.files[i];
          infer_1.findPropRefs(cur.ast, cur.scope, objType, prop.name, storeRef(cur));
        }
      }
  ...
  var _tern = /*#__PURE__*/Object.freeze(/*#__PURE__*/Object.assign(/*#__PURE__*/Object.create(null), tern, {
    'default': tern
  }));
        ...
        if (prev == 10) {
          for (var scan = --pos, sawNonWS = false; scan > 0; --scan) {
            ...
            if (prev == 47 && text.charCodeAt(scan - 2) == 47) {
              ...
              if (!emptyLines && topIsLineComment) found[0] = content + "\n" + found[0];else (found || (found = [])).unshift(content);
  ...
  var comment$1 = /*#__PURE__*/Object.freeze(/*#__PURE__*/Object.assign(/*#__PURE__*/Object.create(null), comment, {
    'default': comment
  }));
  ...
  exports.acorn = acorn$1;
  exports.tern = tern$1;

  Object.defineProperty(exports, '__esModule', { value: true });

})));
//# sourceMappingURL=tern-browser.js.map
